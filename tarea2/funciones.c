...
void show_personas(persona* personas, int cant_personas) {
...
static void Merge(persona *arr, int low, int mid, int high) //ordena danger_category
{
  ...
  {
    if (arr[leftPos].danger_category < arr[rightPos].danger_category) //1ro ordenamos por categoria
    {
      temp[mergePos++] = arr[leftPos++];
    }
    else if((arr[leftPos].danger_category == arr[rightPos].danger_category)
            && arr[leftPos].attack_prob < arr[rightPos].attack_prob)//2do por attack_prob
    {
      temp[mergePos++] = arr[leftPos++];        
    }
    else
